...
int mult(Mat M, Vec U, Vec Y){
	...
	PetscErrorCode ierr;
	InvMedData<FMM_Mat_t> *invmed_data = NULL;
...
//	InvMedTree<FMM_Mat_t>::kernel_ = kernel;
  ...
  // -------------------------------------------------------------------
	...
	PetscInt N = pt_sources->N;
	Mat A;
	InvMedData<FMM_Mat_t> invmed_data;
  ...
  MatCreateShell(comm,m,n,M,N,&invmed_data,&A);
	...
	InvMedData<FMM_Mat_t> *invmed_data2 = NULL;
	MatShellGetContext(A, &invmed_data2);
	std::cout << "After get context, before mult  " << ((invmed_data2->temp)->GetNodeList()).size() << std::endl;
